    ...
    <div class="card-body p-3">
        <form method="GET" class="row g-2">
            <div class="col-sm-4 col-md-3">
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Name or email..." value="<?php echo e(request('search')); ?>">
            </div>
            <div class="col-sm-3 col-md-2">
                <select name="role" class="form-select form-select-sm">
                    <option value="">All Roles</option>
                    <option value="admin" <?php if(request('role')=='admin'): echo 'selected'; endif; ?>>Admin</option>
                    <option value="job_poster" <?php if(request('role')=='job_poster'): echo 'selected'; endif; ?>>Job Poster</option>
            ...
            <div class="col-auto">
                ...
                <a href="<?php echo e(route('admin.users.index')); ?>" class="btn btn-sm btn-outline-secondary">Reset</a>
            </div>
        </form>
    </div>
</div>
                    ...
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div>
                                ...
                                <div class="text-muted" style="font-size:11px"><?php echo e($user->email); ?></div>
                            </div>
                        </div>
                    </td>
                    <td><span class="badge bg-light text-dark border" style="font-size:11px"><?php echo e($user->getRoleNames()->first()); ?></span></td>
                    <td><span class="badge bg-<?php echo e(match($user->status){ 'active'=>'success','suspended'=>'warning','banned'=>'danger', default=>'secondary'}); ?>"><?php echo e(ucfirst($user->status)); ?></span></td>
                    <td><span class="badge bg-<?php echo e($user->verification_status === 'verified' ? 'success' : 'secondary'); ?>" style="font-size:10px"><?php echo e(ucfirst($user->verification_status)); ?></span></td>
                    <td><small class="text-muted"><?php echo e($user->created_at->format('d M Y')); ?></small></td>
                    <td class="text-end">
                        <div class="d-flex gap-1 justify-content-end">
                            <a href="<?php echo e(route('admin.users.show', $user)); ?>" class="btn btn-sm btn-outline-primary" title="View"><i class="fa fa-eye"></i></a>
                            <?php if($user->status === 'active'): ?>
                            <button type="button" class="btn btn-sm btn-outline-warning" title="Suspend" data-bs-toggle="modal" data-bs-target="#suspendUserModal" data-user-id="<?php echo e($user->id); ?>" data-user-name="<?php echo e($user->name); ?>" data-user-route="<?php echo e(route('admin.users.suspend', $user)); ?>"><i class="fa fa-pause"></i></button>
                            <button type="button" class="btn btn-sm btn-outline-danger" title="Ban" data-bs-toggle="modal" data-bs-target="#banUserModal" data-user-id="<?php echo e($user->id); ?>" data-user-name="<?php echo e($user->name); ?>" data-user-route="<?php echo e(route('admin.users.ban', $user)); ?>"><i class="fa fa-ban"></i></button>
                            <?php else: ?>
                            <form method="POST" action="<?php echo e(route('admin.users.activate', $user)); ?>"><?php echo csrf_field(); ?> <button class="btn btn-sm btn-outline-success" title="Activate"><i class="fa fa-check"></i></button></form>
                            <?php endif; ?>
                            <?php if($user->verification_status !== 'verified'): ?>
                            <form method="POST" action="<?php echo e(route('admin.users.verify', $user)); ?>"><?php echo csrf_field(); ?> <button class="btn btn-sm btn-outline-info" title="Mark Verified"><i class="fa fa-id-card"></i></button></form>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\tandi\OneDrive\Desktop\Druk-Freelancing-System\resources\views/admin/users/index.blade.php ENDPATH**/ ?>